...
(() => {
  ...
  const CLIENT_LINKS = {
    clash: (sub, name) => `clash://install-sub?url=${encodeURIComponent(sub)}&name=${encodeURIComponent(name || "sub")}`,
    hiddify: (sub) => `hiddify://import/${encodeURIComponent(sub)}`,
    singbox: (sub) => `sing-box://import-remote?url=${encodeURIComponent(sub)}`,
  ...
  const parseSubscription = (text) => {
    if (!text) return [];
    let working = text;
    // If looks like base64 blob (no protocol prefixes, no newlines), try decode
  ...
  .rc-modal-box { background: rgba(17,24,39,0.96); border: 1px solid rgba(255,255,255,0.1); border-radius: 20px; max-width: 760px; width: 100%; max-height: 86vh; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 30px 70px rgba(0,0,0,0.5); }
  ...
  .rc-btn { display: inline-flex; align-items: center; justify-content: center; gap: 6px; border-radius: 10px; padding: 6px 10px; font-weight: 700; font-size: 11px; border: 1px solid transparent; cursor: pointer; transition: all .15s ease; }
  ...
  .rc-client-row { display: flex; flex-wrap: wrap; gap: 8px; padding: 12px 0; border-bottom: 1px solid rgba(255,255,255,0.06); margin-bottom: 12px; }
    ...
    <div class="rc-modal-box">
      <div class="rc-modal-header">
        <div>
          ...
          <div id="rcModalSub" style="font-size:11px;color:#6b7280;margin-top:2px;"></div>
        </div>
  ...
  const openModal = (title, sub) => {
    ...
    modalBody.innerHTML = `<div style="text-align:center;padding:30px;color:#6b7280;">Loading...</div>`;
  };

  const renderConfigs = (configs, subUrl, subName) => {
    if (!configs.length) {
      modalBody.innerHTML = `<div style="text-align:center;padding:30px;color:#6b7280;">No configs found in this subscription.</div>`;
      return;
    }
    ...
    html += `<div class="rc-client-row">
      <span style="font-size:11px;color:#6b7280;font-weight:700;text-transform:uppercase;letter-spacing:1px;align-self:center;">Import subscription:</span>`;
    for (const [key, fn] of Object.entries(CLIENT_LINKS)) {
      const link = fn(subUrl, subName);
      html += `<a href="${esc(link)}" class="rc-btn rc-btn-primary" style="text-transform:capitalize;">${esc(key)}</a>`;
    }
    html += `<button class="rc-btn rc-btn-secondary" id="rcCopySub">Copy URL</button>`;
    html += `</div>`;

    html += `<div style="font-size:11px;color:#6b7280;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">${configs.length} configs</div>`;
  ...
  const showConfigs = async (subUrl, name) => {
    ...
    try {
      ...
      modalBody.innerHTML = `<div style="text-align:center;padding:30px;color:#f87171;">Failed to fetch: ${esc(e.message)}<br><br><span style="color:#6b7280;font-size:12px;">CORS or network error. Try the "Source" button to open the raw file.</span></div>`;
    }
  };
  ...
  const pingHost = async (host, port) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      try {
        // no-cors: opaque response, but TCP/TLS handshake time is measured
        ...
        // fetch throws on opaque too sometimes; measure time anyway
        const t1 = performance.now();
        // If it threw quickly, host may be unreachable; if slowly, reachable but opaque
        samples.push(t1 - t0);
      }
    }
  ...
  const augmentSubCard = (card) => {
    ...
    // find the button grid
    const grid = copyBtn.parentElement;
    if (grid) {
      // insert after the grid as full-width
  ...
  const augmentProxyCard = (card) => {
    ...
    // insert into the ping row (the .flex.items-center.gap-2.mt-3)
  ...
  if (document.readyState === "loading") {
    ...
    // wait a tick for the dashboard to render
